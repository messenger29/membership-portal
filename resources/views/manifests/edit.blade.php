...
<h1>Edit Race Manifest: {{ $race_crew->name }}</h1>
<p>Team: {{ $team->name }}</p>
<p>Race: {{ $race_event->name }}</p>
{{ Form::open(['action' => ['ManifestsController@update', $race_crew->id], 'method' => 'PUT']) }}
<div class="well">
	<ul style="font-size: 18px;">
		@foreach($roster as $member)
			@if($member->rostered == -1)
				<li>{{ $member->name }}</li>
			@elseif($member->rostered == 1)
				<li>{{ Form::checkbox('members[]', $member->id, true, []) }} {{ $member->name }}</li>
			@else
